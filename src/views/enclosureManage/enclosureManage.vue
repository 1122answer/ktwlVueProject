        ...
        <v-row>
           ...
           <v-col span="20">
               ...
               <!--       <v-form-item label="业务名称">
                         <v-input placeholder="请输入业务名称" :style="{width:'100%'}"></v-input>
                     </v-form-item> -->
                     <v-form-item label="要件标题">
                          <v-input placeholder="请输入规则名称" v-model="title" :style="{width:'100%'}"></v-input>
                     </v-form-item>
                     <v-form-item label="要件类型">
                         <v-select placeholder="请选择系统" v-model="cconfigType" style="width:150px" :data="[{value:'复印件',label:'复印件'},{value:'原件',label:'原件'}]"></v-select>
                     </v-form-item>
                     <v-form-item label="文档类型">
                         <v-select  style="width:150px" v-model="fileType" :data="FiletypeSelect"></v-select>
                     </v-form-item>
                     <v-form-item label="是否开启">
                         <v-select style="width:150px" v-model="isEnabled" :data="[{value:'true',label:'是'},{value:'false',label:'否'}]"></v-select>
                     </v-form-item>
                 </v-form>
                  <v-button slot="control" size="large" type="primary" html-type="button" icon="search" @click="searchlist">查询</v-button>
                  <v-button type="primary" size="large" class="pull-right" style="margin-top:2px;" @click="addEncosuer"><v-icon type="plus"></v-icon> 添加</v-button>
                  </v-more-panel>
                     <v-data-table  class="margin-top-15" :data='loadData' size="small" :columns='columns' ref="encTable" bordered emptyText="暂时找不到你要的信息......">
    ...
    methods: {
      // 点击树
        selectFu(param) {
          ...
          // 详情
          detaillist(event){
            this.$store.commit("enclosureAddModule/DETAILFORM",event);
            this.visible=true;
          },
          hideDialog(){
            this.visible=false;
