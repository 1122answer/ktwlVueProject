...
export default {
    ...
    computed: {
        ...mapState({
            treeData: state => state.deptRoleModule.treeData,
            dataRow: state => state.deptRoleModule.rowVal
        })
    },
    data() {
        return {
        ...
        handleOk() {
            ...
            if (this.checkRoleArr.length > 0) {
                this.checkRoleArr.map(item => {
                    ...
                    obj.name = item.name;
                    obj.relationType = 1;
                    obj.relationTypeId = this.dataRow.deptId;
                    saveArr.push(obj);
                });
            } else {
                var obj = {};
            ...
            this.$store.dispatch('deptRoleModule/saveDeptRole', saveArr).then(res => {
