                ...
                <v-more-panel>
                    <v-form slot="form">
                        <v-form-item label="字典名称">
                            <v-input placeholder="请输入字典名称" v-model="searchForm.dictionaryName"></v-input>
                        </v-form-item>
                          <v-form-item label="字典参数">
                            <v-input placeholder="请输入字典参数" v-model="searchForm.dictionaryKey"></v-input>
                        </v-form-item>
                        <v-form-item label="调用方式">
                            <v-select placeholder="请选择调用方式" style="width: 120px;" :data="[{value:null,label:'全部'},{value:'0',label:'正常'},{value:'1',label:'接口'},{value:'2',label:'数据库'}]" v-model="searchForm.dictType">
                            </v-select>
                        </v-form-item>
                       <v-form-item label="是否开启">
                        ...
                        <v-button type="primary" @click="searchList"> <v-icon type="search"></v-icon> 查询</v-button>
                    </v-form>
                     <v-button type="primary" @click="addParams" class="margin-left-5" title="新增字典组和项">添加字典</v-button>
                     <v-button type="primary"  class="margin-left-5" title="新增字典组和项" :disabled="true">添加项</v-button>
                </v-more-panel>

                </div>
                <div class="container-fluid clearfix" style="min-width: 1000px;">
                    <v-data-table class="margin-top-15" :data='loadData' size="small" :columns='columns' ref="dicTable" bordered @clickrow="clickrowFu" :currentData.sync ="currentData" emptyText="暂时找不到你要的信息......">
        ...
        copygroup(data) {
            ...
            if (this.copyedgroup) {
                ...
                this.$store.dispatch("dicAddModule/copynumberList", this.dictionaryId).then(res => {
                   this.$store.dispatch('dicAddModule/fetchDicKeys', {
                       }).then(res => {
                        this.copyShow = true;  
                    });
                });
