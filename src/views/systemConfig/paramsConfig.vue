    ...
    <v-row>
      ...
      <v-col span="20">
        ...
        <div class="container-fluid clearfix" style="min-width: 1000px;">
          <v-data-table class="margin-top-15" :data='loadData' size="small" :columns='columns' ref="paramsTable" bordered emptyText="暂时找不到你要的信息......">
...
export default {
  ...
  data: function() {
    return {
      ...
      loadData(params) {
        return this.$store
          .dispatch("prsListModule/fetchParansTable", {
  ...
  methods: {
    ...
    selectFu(selectedNodes) {
        ...
        this.copyregionstate = false;
        this.$store.dispatch("copyRegionParamsModule/getGroupList",selectNode)
    ...
    addParams() {
      ...
      // this.$store.commit("prsAddEditModule/UPDATE_ADD_DEFAULT_DATA", this.selectNode);
    },
    // copyParams() {
    //   if(this.selectNode.nodeType == 'region'){
    //     this.crpvisible = true;
    //     this.$store.commit("copyRegionParamsModule/INIT_FORM_DATA",this.selectNode);
    //     this.$store.commit("copyRegionParamsModule/INIT_DES_SYSTEM_DATA", this.systemData);
    //     this.$store.commit("copyRegionParamsModule/INIT_DES_REGION_DATA", this.regionData);
