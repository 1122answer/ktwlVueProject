<template>
    <v-modal :title="titleName" :visible="visible" @cancel="cancel" :width="800" :confirmLoading="rolebutstate" :maskClosable="false">
            ...
            <v-row>
                <v-col span='12'>
                    <v-form-item label="所属系统" :label-col="labelCol" :wrapper-col="wrapperCol">
                        ...
                        <v-select v-else v-model="paramConfigForm.systemId" placeholder="请选择所属系统" notfound="无法找到" :data="systemData" size="lg" label="label" :optionOnChange="true" @change="systemChange" ref="selectSystem"></v-select>
                    </v-form-item>
                </v-col>
                <v-col span='12'>
                    <v-form-item label="行政区域名称" :label-col="labelCol" :wrapper-col="wrapperCol">
                        <span v-if="addFlag == 1">{{paramConfigForm.regionName}}</span>
                        <v-select v-else v-model="paramConfigForm.systemId" placeholder="请选择区域" notfound="无法找到" :data="regionData" size="lg" label="label" :optionOnChange="true" @change="regionChange" ref="selectRegion"></v-select>
                    </v-form-item>
                </v-col>
            </v-row>
            <v-row>
